	...
	$.fn.fullpage = function(options) {
		...
		*/
		...
		$.fn.fullpage.moveSlideRight = function(){
			if(activeSection.find('.fp-slides').length) moveSlide('next');
		};

		$.fn.fullpage.moveSlideLeft = function(){
			if(activeSection.find('.fp-slides').length) moveSlide('prev');
		};
					...
					if (Math.abs(touchStartX - touchEndX) > (Math.abs(touchStartY - touchEndY))) {
					    ...
					    if (Math.abs(touchStartX - touchEndX) > ($(window).width() / 100 * options.touchSensitivity)) {
					        if (touchStartX > touchEndX) {
								$.isFunction( options.onSlideRight ) && options.onSlideRight.call(container);
					        	if(isScrollAllowed.right){
					            	$.fn.fullpage.moveSlideRight(); //next
					            }
					        } else {
								$.isFunction( options.onSlideLeft ) && options.onSlideLeft.call(container);
